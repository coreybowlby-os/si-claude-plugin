...
function runTests() {
  ...
  if (test('finds current plugin install at ~/.claude/plugins/SI-Claude-Plugin@ecc', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('finds marketplace current plugin install at ~/.claude/plugins/marketplaces/SI-Claude-Plugin', () => {
    const homeDir = createTempDir();
    try {
  ...
  if (test('finds marketplace legacy plugin install at ~/.claude/plugins/marketplaces/SI-Claude-Plugin', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'SI-Claude-Plugin']);
      const result = resolveEccRoot({ envRoot: '', homeDir });
  ...
  if (test('prefers exact legacy plugin install over plugin cache', () => {
    ...
    try {
      const expected = setupLegacyPluginInstall(homeDir, ['marketplaces', 'SI-Claude-Plugin']);
      setupPluginCache(homeDir, 'SI-Claude-Plugin', 'affaan-m', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('discovers plugin root from cache directory', () => {
    const homeDir = createTempDir();
    try {
      const expected = setupPluginCache(homeDir, 'SI-Claude-Plugin', 'affaan-m', CURRENT_PACKAGE_VERSION);
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, expected);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('rejects a partial exact plugin root (scripts, no ECC skill) and prefers a complete root (#2544)', () => {
    const homeDir = createTempDir();
    try {
      // An exact plugin root under ~/.claude/plugins/SI-Claude-Plugin ships ECC's scripts but
      // not ECC's skills. The stricter predicate must reject it on the
      // exact-plugin branch too, not only for ~/.claude.
      const partialScripts = path.join(homeDir, '.claude', 'plugins', 'SI-Claude-Plugin', 'scripts', 'lib');
  ...
  if (test('rejects a partial plugin-cache root (scripts, no ECC skill) and falls back to ~/.claude (#2544)', () => {
    ...
    try {
      ...
      const cacheScripts = path.join(
        homeDir, '.claude', 'plugins', 'cache', 'SI-Claude-Plugin', 'affaan-m', CURRENT_PACKAGE_VERSION,
        'scripts', 'lib'
      );
  ...
  if (test('custom probe skips a qualifying root that lacks the probed script (auto-update)', () => {
    ...
    try {
      const claudeDir = setupStandardInstall(homeDir);
      const marketplaceRoot = setupLegacyPluginInstall(homeDir, ['marketplaces', 'SI-Claude-Plugin']);
...
module.exports = { resolveEccRoot() { assert.strictEqual(process.env.HOME, ${JSON.stringify(homeDir)}); return 'module-loaded'; } };`);
  ...
  if (test('INLINE_RESOLVE bootstraps module from an exact plugin root when env unset', () => {
    const homeDir = createTempDir();
    try {
      const resolverDir = path.join(homeDir, '.claude', 'plugins', 'SI-Claude-Plugin', 'scripts', 'lib');
      fs.mkdirSync(resolverDir, { recursive: true });
