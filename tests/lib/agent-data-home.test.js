 ...
 * from the real project root intentionally resolves to `~/.cursor/SI-Claude-Plugin` — see the
...
function runTests() {
  ...
  if (test('resolveAgentDataHome uses projectDir + .cursor/ecc-agent-data.json', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ agentDataHome: '~/.cursor/SI-Claude-Plugin' }),
      'utf8'
    );
      ...
      withIsolatedCwd(() => {
        withEnv({
          ...
          assert.strictEqual(
            ...
            path.join(os.homedir(), '.cursor', 'SI-Claude-Plugin')
          );
        });
      });
    } finally {
  ...
  if (test('defaults to ~/.cursor/SI-Claude-Plugin in Cursor hook runtime (isolated cwd)', () => {
    withIsolatedCwd(() => {
      withEnv({
        ECC_AGENT_DATA_HOME: undefined,
        ...
        assert.strictEqual(
          agentDataHome.resolveAgentDataHome(),
          path.join(home, '.cursor', 'SI-Claude-Plugin')
        );
      });
    });
  })) passed++; else failed++;
  ...
  if (test('reads project ecc-agent-data.json config file', () => {
    ...
    const customHome = path.join(homeDir, '.cursor', 'SI-Claude-Plugin', 'custom');
    ...
    try {
      withEnv({
        ...
        assert.strictEqual(
          ...
          path.join(fs.realpathSync(homeDir), '.cursor', 'SI-Claude-Plugin', 'custom')
        );
      });
    } finally {
    ...
    try {
      ...
      withEnv({
        ...
        assert.strictEqual(
          ...
          path.join(homeDir, '.cursor', 'SI-Claude-Plugin')
        );
      });
    } finally {
      process.chdir(originalCwd);
  ...
  if (test('rejects relative project config paths even when the project is beneath the trusted root', () => {
    ...
    const projectDir = path.join(homeDir, '.cursor', 'SI-Claude-Plugin', 'checked-out-project');
    const configPath = path.join(projectDir, '.cursor', 'ecc-agent-data.json');
    ...
    try {
      withEnv({
        ...
        const unsafeCandidates = [
          ...
          '~/.cursor/SI-Claude-Plugin/profiles/../traversed-data',
  ...
  if (test('allows a non-existent project config destination beneath the Cursor data root', () => {
    ...
    const configPath = path.join(projectDir, '.cursor', 'ecc-agent-data.json');
    const safeHome = path.join(homeDir, '.cursor', 'SI-Claude-Plugin', 'profiles', 'work');
    fs.mkdirSync(path.dirname(configPath), { recursive: true });
    ...
    try {
      withEnv({
        ...
        assert.strictEqual(
          agentDataHome.readProjectConfigAt(configPath),
          path.join(fs.realpathSync(homeDir), '.cursor', 'SI-Claude-Plugin', 'profiles', 'work')
        );
      });
    } finally {
      fs.rmSync(projectDir, { recursive: true, force: true });
  ...
  if (test('rejects a project config destination that escapes through a symlink', () => {
    ...
    const configPath = path.join(projectDir, '.cursor', 'ecc-agent-data.json');
    const cursorRoot = path.join(homeDir, '.cursor', 'SI-Claude-Plugin');
