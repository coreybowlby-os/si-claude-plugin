...
function uninstallArgv(scope) {
  return ['plugin', 'uninstall', 'SI-Claude-Plugin@SI-Claude-Plugin', '--scope', scope, '--keep-data'];
}
...
test('zero installs, ambiguous non-destination scopes, and invalid inventories fail closed', () => {
  const cases = [
    ...
    {
      state: { pluginListResponses: [[{ id: 'SI-Claude-Plugin@SI-Claude-Plugin', enabled: true }]] },
      scope: 'project',
      code: 'INVALID_PLUGIN_INVENTORY',
    },
    {
  ...
  const cases = [
    {
      state: {
        ...
        marketplaces: [marketplace('user')],
        failures: [{
          argv: ['plugin', 'marketplace', 'update', 'SI-Claude-Plugin'],
          status: 7,
...
test('migration preserves hook preferences unless --hooks is explicit', () => {
  ...
  withFixture({
    ...
    assert.strictEqual(settings.pluginConfigs['SI-Claude-Plugin@SI-Claude-Plugin'].futureKey, true);
