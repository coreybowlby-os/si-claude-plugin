...
function runTests() {
  ...
  if (test('resolves claude adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(statePath, path.join(homeDir, '.claude', 'SI-Claude-Plugin', 'install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans namespaced Claude rules and flat discoverable skills', () => {
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.claude', 'rules', 'SI-Claude-Plugin')
      )),
  ...
  if (test('resolves claude-project adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.claude', 'SI-Claude-Plugin', 'install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans project-scoped namespaced Claude rules and flat skills', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.claude', 'SI-Claude-Plugin', 'install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
        && operation.destinationPath === path.join(projectRoot, '.claude', 'rules', 'SI-Claude-Plugin')
      )),
