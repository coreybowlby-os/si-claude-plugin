...
async function runTests() {
  ...
  if (await test('creates private state-store directories and atomically persists a private database file', async () => {
    ...
    const privateParent = path.join(testDir, 'new-parent', 'SI-Claude-Plugin');
  ...
  if (await test('refuses an intermediate state database symlink without writing outside the requested tree', async () => {
    ...
    const dbPath = path.join(linkedParent, 'SI-Claude-Plugin', 'state.db');
    ...
    try {
      ...
      assert.strictEqual(fs.existsSync(path.join(outsideDir, 'SI-Claude-Plugin', 'state.db')), false);
    } finally {
      cleanupTempDir(testDir);
    }
  })) passed += 1; else failed += 1;
