...
const MARKETPLACE_UPGRADE = [
  'plugin', 'marketplace', 'upgrade', 'si-claude-plugin', '--json',
];
const PLUGIN_ADD = ['plugin', 'add', 'si-claude-plugin@si-claude-plugin', '--json'];

function marketplaceInventory(installed = false) {
  return JSON.stringify({
    marketplaces: installed ? [{ name: 'si-claude-plugin', root: '/cache/si-claude-plugin' }] : [],
  });
}

function pluginInventory(installed = false, overrides = {}) {
  const ecc = {
    pluginId: 'si-claude-plugin@si-claude-plugin',
    name: 'si-claude-plugin',
    marketplaceName: 'si-claude-plugin',
    version: '2.0.0',
    installed: true,
    enabled: true,
    ...overrides,
...
function marketplaceUpgradeResult(overrides = {}) {
  return JSON.stringify({
    selectedMarketplaces: ['si-claude-plugin'],
    upgradedRoots: ['/cache/si-claude-plugin'],
    errors: [],
    ...overrides,
  });
}
...
function dependenciesFor(fake, overrides = {}) {
  return {
    ...
    resolveMarketplaceRepository: async () => 'https://github.com/coreybowlby-os/si-claude-plugin.git',
    ...overrides,
  };
}
  ...
  const cases = [
    ['parses current Codex marketplace and plugin JSON inventory shapes', () => {
      assert.deepStrictEqual(
        ...
        [{ name: 'si-claude-plugin', root: '/cache/si-claude-plugin' }]
      );
      assert.strictEqual(
    ...
    ['resolves marketplace provenance with execFile and exact Git argv', async () => {
      const fake = createExecFile([{
        command: 'git',
        args: ['-C', '/cache/si-claude-plugin', 'remote', 'get-url', 'origin'],
        stdout: 'https://github.com/coreybowlby-os/si-claude-plugin.git\n',
      }]);
      ...
      assert.strictEqual(repository, 'https://github.com/coreybowlby-os/si-claude-plugin.git');
    ...
    ['fresh install uses exact native Codex argv and verifies both mutations', async () => {
      const fake = createExecFile([
        ...
        { args: PLUGIN_ADD, stdout: '{"pluginId":"si-claude-plugin@si-claude-plugin"}' },
    ...
    ['accepts the provider root across Windows separator and case differences', () => {
      const result = parseMarketplaceUpgradeResult(
        ...
        { name: 'si-claude-plugin', root: 'C:\\Users\\Hira\\.codex\\marketplaces\\ecc' }
      );

      assert.deepStrictEqual(result.selectedMarketplaces, ['si-claude-plugin']);
    }],
      ...
      assert.throws(
        () => parseMarketplaceUpgradeResult(
          marketplaceUpgradeResult({
            ...
            upgradedRoots: ['/cache/si-claude-plugin', '/cache/other'],
          }),
          { name: 'si-claude-plugin', root: '/cache/si-claude-plugin' }
        ),
        error => (
        ...
        reconcileCodexPlugin({}, dependenciesFor(fake, {
          resolveMarketplaceRepository: async () => {
            ...
            return provenanceChecks === 1
              ? 'https://github.com/coreybowlby-os/si-claude-plugin.git'
      ...
      await expectSetupError(
        reconcileCodexPlugin({}, dependenciesFor(fake, {
          resolveMarketplaceRepository: async marketplace => {
            assert.strictEqual(marketplace.root, '/cache/si-claude-plugin');
            return 'https://github.com/attacker/ecc.git';
          },
        })),
        'MARKETPLACE_COLLISION',
        /refusing.*si-claude-plugin.*marketplace/i
      );
      assert.deepStrictEqual(fake.calls.map(call => call.args), [
        MARKETPLACE_LIST,
        PLUGIN_LIST,
    ...
    ['rejects malformed marketplace and plugin JSON inventories', async () => {
      ...
      assert.throws(
        () => parseMarketplaceInventory(JSON.stringify({
          marketplaces: [
            { name: 'si-claude-plugin', root: '/one' },
            { name: 'si-claude-plugin', root: '/two' },
          ],
        })),
        error => error.code === 'INVALID_MARKETPLACE_INVENTORY'
      );
      ...
      assert.throws(
        () => parsePluginInventory(JSON.stringify({
          installed: [
            { pluginId: 'si-claude-plugin@si-claude-plugin', installed: true, enabled: true },
            { pluginId: 'si-claude-plugin@si-claude-plugin', installed: true, enabled: true },
          ],
          available: [],
        })),
    ...
    ['fails when post-install verification does not find enabled ECC', async () => {
      ...
      await expectSetupError(
        ...
        /verify.*si-claude-plugin@si-claude-plugin/i
      );
    }],
