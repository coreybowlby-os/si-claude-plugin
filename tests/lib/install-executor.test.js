...
function runTests() {
  ...
  if (test('plans Claude legacy rules under the default ECC-managed rules directory', () => {
    ...
    try {
      ...
      const managedRulesDir = path.join(homeDir, '.claude', 'rules', 'SI-Claude-Plugin');
      assert.strictEqual(plan.installRoot, managedRulesDir);
      assert.ok(operationFor(plan, path.join('.claude', 'rules', 'SI-Claude-Plugin', 'common', 'coding-style.md')));
      assert.ok(operationFor(plan, path.join('.claude', 'rules', 'SI-Claude-Plugin', 'typescript', 'testing.md')));
  ...
  if (test('materializes manifest scaffold operations and filters generated runtime state', () => {
    ...
    try {
      ...
      assert.ok(plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.claude', 'rules', 'SI-Claude-Plugin', 'common', 'coding-style.md')
      )));
      assert.ok(plan.operations.some(operation => (
  ...
  if (test('applyInstallPlan re-export applies a manifest plan and writes install state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(homeDir, '.claude', 'rules', 'SI-Claude-Plugin', 'common', 'coding-style.md')));
      ...
      const state = JSON.parse(fs.readFileSync(path.join(homeDir, '.claude', 'SI-Claude-Plugin', 'install-state.json'), 'utf8'));
