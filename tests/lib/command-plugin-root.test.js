...
test('resolveEccRoot module covers current and legacy marketplace plugin roots', () => {
  ...
  try {
    const legacyRoot = path.join(legacyHomeDir, '.claude', 'plugins', 'marketplaces', 'SI-Claude-Plugin');
  ...
  try {
    const cacheRoot = path.join(cacheHomeDir, '.claude', 'plugins', 'cache', 'SI-Claude-Plugin', 'coreybowlby-os', '1.0.0');
