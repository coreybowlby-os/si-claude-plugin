...
if (args[0] === 'plugin' && args[1] === 'marketplace' && args[2] === 'add') {
  ...
  const marketplaces = [
    ...(state.marketplaces || []).filter(entry => entry.name !== 'SI-Claude-Plugin'),
    {
...
if (args[0] === 'plugin' && args[1] === 'install' && args[2] === 'SI-Claude-Plugin@SI-Claude-Plugin') {
  const scopeIndex = args.indexOf('--scope');
  const scope = scopeIndex >= 0 ? args[scopeIndex + 1] : 'user';
  const plugins = [
    ...(state.plugins || []).filter(plugin => (
      plugin.id !== 'SI-Claude-Plugin@SI-Claude-Plugin' || plugin.scope !== scope
    )),
    { id: 'SI-Claude-Plugin@SI-Claude-Plugin', scope, enabled: true, version: '2.0.0' },
  ];
  writeState({ ...state, plugins });
  process.exit(0);
}

if (args[0] === 'plugin' && args[1] === 'update' && args[2] === 'SI-Claude-Plugin@SI-Claude-Plugin') {
  const scopeIndex = args.indexOf('--scope');
  const scope = scopeIndex >= 0 ? args[scopeIndex + 1] : 'user';
