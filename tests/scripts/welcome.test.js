...
test('ecc welcome renders the install artwork for captured agent output', () => {
  ...
  assert.match(result.stdout, /Welcome to SI Claude Plugin!/);
  assert.ok(result.stdout.includes(`v${version}`));
  assert.match(result.stdout, /GitHub:\s+https:\/\/github\.com\/coreybowlby-os\/si-claude-plugin/);
...
test('ecc welcome supports explicit update and configured outcomes', () => {
  const cases = [
    ['updated', /SI Claude Plugin is updated/],
    ['configured', /SI Claude Plugin is configured/],
    ['migrated', /SI Claude Plugin is configured/],
    ['resumed', /SI Claude Plugin is configured/],
    ['already-migrated', /SI Claude Plugin is configured/],
  ];
