...
```bash
sicp memory init --scope project --scope team
```
...
```bash
sicp memory search "research parser" --target-harness codex
...
0. Run `sicp migrate audit --source ~/.hermes` first to inventory the legacy workspace and see which parts already map onto ECC2.
0.5. Plan and scaffold migration artifacts before importing anything:
   - generate reviewable plans with `sicp migrate plan` and `sicp migrate scaffold`
   - scaffold reusable legacy skills with `sicp migrate import-skills --output-dir migration-artifacts/skills`
   - scaffold tool translation templates with `sicp migrate import-tools --output-dir migration-artifacts/tools`
   - scaffold bridge plugin templates with `sicp migrate import-plugins --output-dir migration-artifacts/plugins`
   - preview recurring jobs with `sicp migrate import-schedules --dry-run`
   - preview gateway dispatch with `sicp migrate import-remote --dry-run`
   - preview safe env/service context with `sicp migrate import-env --dry-run`
   - import sanitized workspace memory with `sicp migrate import-memory`
...
3. Initialize the shared ECC Memory Vault. Register `ecc-memory-mcp` only if
   Hermes needs tool access instead of the `sicp memory` CLI.
