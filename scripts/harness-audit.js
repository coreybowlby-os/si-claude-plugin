...
// Upstream carries two live slugs ('ecc' and 'everything-claude-code'); we ship exactly one,
// so each list below holds a single form rather than a pair. We deliberately do NOT detect
// upstream's slugs: reaching into a namespace we do not own is the same problem that got
// 'ecc' dropped from these lists originally. We have never shipped, so there is no legacy
// install of ours to migrate either.
...
const ECC_CACHE_MARKETPLACES = ['SI-Claude-Plugin'];
const ECC_CACHE_PLUGIN_NAMES = ['SI-Claude-Plugin'];
