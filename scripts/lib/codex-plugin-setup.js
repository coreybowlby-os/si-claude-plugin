...
// Codex plugin ids are `<plugin>@<marketplace>`; both halves are our lowercase
// Codex-surface name.
const OFFICIAL_MARKETPLACE_NAME = CODEX_MARKETPLACE_NAME;
const CODEX_PLUGIN_ID = `${CODEX_MARKETPLACE_NAME}@${OFFICIAL_MARKETPLACE_NAME}`;
...
async function assertOfficialMarketplace(
  ...
  if (repository !== NORMALIZED_OFFICIAL_MARKETPLACE_REPO) {
    fail(
      'MARKETPLACE_COLLISION',
      `Refusing the existing \`${OFFICIAL_MARKETPLACE_NAME}\` marketplace because it is not the official ${OFFICIAL_MARKETPLACE_REPO} source.`,
      { phase }
    );
  }
}
