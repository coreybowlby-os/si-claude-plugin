...
function parsePluginList(stdout) {
  ...
  for (const plugin of plugins) {
    // Only our own entries are validated strictly. A malformed upstream-ECC entry in the
    // user's inventory must not block our setup — ECC is allowed to coexist.
    const isRelevant = plugin && (
      plugin.id === CURRENT_PLUGIN_ID
      || String(plugin.id || '').startsWith(`${CURRENT_PLUGIN_SLUG}@`)
    );
    ...
    if (
      ...
      fail(
        ...
        `Claude plugin inventory contains an invalid ${CURRENT_PLUGIN_ID} entry`
      );
    }
  }
  return plugins;
  ...
  for (const marketplace of marketplaces) {
    ...
    if (
      ...
      fail(
        ...
        `Claude marketplace inventory contains an invalid \`${OFFICIAL_MARKETPLACE_NAME}\` entry`
      );
    }
  }
  return marketplaces;
...
// Refuses a SECOND copy of this plugin registered under a different marketplace — that
// genuinely duplicates our skills, agents and commands on disk. Upstream ECC is not that:
// it is a separate plugin and is allowed to coexist, so nothing here matches `ecc@*` or
// `everything-claude-code@*`.
function assertNoConflictingEccPlugins(plugins) {
  const duplicate = plugins.find(plugin => (
    typeof plugin?.id === 'string'
    && plugin.id.startsWith(`${CURRENT_PLUGIN_SLUG}@`)
    && plugin.id !== CURRENT_PLUGIN_ID
  ));
  if (duplicate) {
    fail(
      'DUPLICATE_PLUGIN',
      `${duplicate.id} is already installed and would duplicate ${CURRENT_PLUGIN_ID} surfaces. Uninstall it before setting up ${CURRENT_PLUGIN_ID}.`,
      {
        observedScopes: [duplicate.scope],
        recovery: [
          `claude plugin uninstall ${duplicate.id} --scope ${duplicate.scope} --keep-data`,
        ],
      }
    );
  }
...
function ensureOfficialMarketplace(options) {
  ...
  if (existing && !isOfficialMarketplace(existing)) {
    fail(
      ...
      `Refusing the \`${OFFICIAL_MARKETPLACE_NAME}\` marketplace collision because it is not the official ${OFFICIAL_MARKETPLACE_REPO} source.`
    );
  }
  ...
  if (!verified || !isOfficialMarketplace(verified)) {
    fail(
      ...
      `Could not verify the official ${OFFICIAL_MARKETPLACE_NAME} marketplace after the marketplace change.`,
